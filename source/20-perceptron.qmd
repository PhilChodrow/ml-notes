...
\end{aligned}
$$
- A *target variable* $y_i$. In this data set, the target variable is has components equal to either $0$ or $1$. [For the perceptron, it turns out to be extra convenient to use targets of $\{-1, 1\}$ instead; we'll handle this by transforming a target $y$ into $2y-1$ which has the right values.]{.aside} You can think of it as representing a piece of yes-no information like whether a student is a computer science major or not. The target variables can be collected into a *target vector* $\mathbf{y} = (y_1, \ldots, y_{n})^T \in \{0,1\}^{n}$. 
...
That is, if someone gave you the weight vector $\mathbf{w}^{(1)}$, ***you wouldn't need to see the data labels***: you could exactly recover them using @eq-perceptron-predict. Not every data set can be separated this way, but we have a term of art for those that can: 
...
A data set with feature matrix $\mathbf{X} \in \mathbb{R}^{n\times k}$ and target vector $y\in \{0, 1\}$ is *linearly separable* if there exists a weight vector $\mathbf{w}$ such that, for all $i \in [n]$, 
...
[We've used a little math trick here: take a moment to convince yourself that @eq-perceptron-predict is equivalent to the statement that $s_i (2y_i-1) > 0$.]{.aside}
...
2. "Until we're done," in each time-step $t$, 
    ...
    - Else, if $s^{(t)}_i (2y_i-1) < 0$, then perform the update 
    $$\mathbf{w}^{(t+1)} = \mathbf{w}^{(t)} + (2y_i-1) \mathbf{x}_i\;.$$ {#eq-perceptron-update}
...
Let's first check that the perceptron update in @eq-perceptron-update actually improves the prediction on data point $i$ if there is currently a mistake on that point (i.e. if $s^{(t)}_i y_i < 0$). We can do this by computing the new $s_i^{(t+1)}$. Remember, what we want is for the sign of $s_i^{(t+1)}$ to match $2y_i-1$. 

$$
\begin{align}
s_i^{(t+1)} &= \langle \mathbf{w}^{(t+1)}, \mathbf{x}_i\rangle  &\text{(definition of $s_i^{(t+1)}$)}\\ 
               &= \langle \mathbf{w}^{(t)} + (2y_i-1)\mathbf{x}_i, \mathbf{x}_i\rangle &\text{(perceptron update)} \\ 
               &= \langle \mathbf{w}^{(t)},\mathbf{x}_i\rangle + (2y_i-1)\langle \mathbf{x}_i, \mathbf{x}_i\rangle &\text{(linearity of $\langle \cdot\rangle$)}\\ 
               &= s_i^{(t)} + (2y_i-1) \lVert \mathbf{x}_i\rVert_2^2\;. &\text{(definition of $s_i^{(t)}$ and $\lVert \mathbf{x}_i\rVert$)} 
\end{align}
$$

[Again, this is only if $s^{(t)}_i (2y_i-1) < 0$; otherwise there is no change in $s_i^{(t)}$ in the current iteration.]{.aside}

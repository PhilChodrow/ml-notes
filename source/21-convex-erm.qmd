...
The zero-one loss looks like this: 

```{python}
fig, axarr = plot_loss(loss_fun = zero_one_loss)
```
...
The purpose of introducing convexity in this context is to replace the nonconvex 0-1 loss function with a convex substitute. [Such functions are also sometimes called *surrogate* loss functions.]{.aside} Here's an example, which is usually called the *hinge loss*, which is defined by the formula 
...
#---
def hinge(s, y): 
    return y * torch.max(torch.zeros_like(s), s)  + (1-y) * torch.max(torch.zeros_like(s), -s)
#---
```
...
Another loss function is the sigmoid binary cross entropy, which is defined by the formula 
...
4. **Performance**: If it's possible to find the best solution, can we do so efficiently using simple algorithms?
...
Now we'll use the `minimize` function from `scipy.optimize` to find the value of $\mathbf{w}$ that makes this function smallest: [We initialize `w0` as a Numpy array for compatibility reasons with `scipy.optimize.minimize`.]{.aside}

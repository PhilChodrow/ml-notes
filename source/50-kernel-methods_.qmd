...
Thus, $\mathbf{w}^{(1)}$ is a linear combination of the feature vectors $\phi(\mathbf{x}_i)$. 
...
```{python}
KR = KernelRidgeRegression(rbf_kernel, lam = .1, gamma = 0.03)
KR.fit(X, y)
```
...
Much nicer! As usual, increasing $\gamma$ too much results in undesirably wiggly models that may reflect overfitting. 

```{python}
KR = KernelRidgeRegression(rbf_kernel, lam = .1, gamma = 10)
KR.fit(X, y)
plot_pattern(X, y, KR, color = "black")
```
...
labs = plt.gca().set(xlabel = "True Review Rating", ylabel = "Predicted Review Rating")
```

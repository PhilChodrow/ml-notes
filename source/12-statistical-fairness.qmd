...
In these notes, we'll review three fundamental definitions of fairness for decision-making systems. While texts like @barocasFairnessMachineLearning2023 couch their definitions in the language of probability, our focus in these notes will be to relate this language to operational computations on data sets in Python. We'll practice some of our skills with `pandas`, `numpy`, and `seaborn`. After reviewing these definitions, we'll review a simple, famous result about the ability of decision systems to satisfy multiple definitions. 
...
For our present purposes, we focus on the definition of independence for binary classifiers as given by @barocasFairnessMachineLearning2023. 
...
Colloquially, @def-independence says that the probability of a positive prediction $\hat{Y} = 1$ does not depend on the group membership $A$. In the COMPAS data, independence would require that the probability of the model predicting that an individual will be arrested within two years be the same for white and Black defendants. 
...
Let's write another function with the same API to give a summary of error rates between two groups using these formulas. As we saw last time, it's pretty convenient to do this with confusion matrices. It's not much more difficult to do it "by hand" using vectorized Pandas computations: 
...
The false positive rates are in the rows in which `two_year_recid == 0`, and the true positive rates are in the rows in which `two_year_recid == 1`. If you are playing along at home, you'll notice that these error rates exactly match the rates we calculated via confusion matrices in our first analysis of the COMPAS data set. 
...
The quantity $\mathbb{P}(Y = 1 | \hat{Y} = 1, A = a)$ is sometimes called the *positive predictive value* (PPV) of $\hat{Y}$ for group $a$. Similarly, $\mathbb{P}(Y = 0 | \hat{Y} = 0, A = a)$ is sometimes called the *negative predictive value* (NPV) of $\hat{Y}$ for group $a$. So, the sufficiency criterion demands that the positive and negative predictive values be equal across groups. 
...
def test_sufficiency(df, group_col, target, pred):
    ...
    return df_.groupby([group_col, pred])["correct"].mean().reset_index()
#---
```
...
The proof strategy is to derive an algebraic relationship between the error rates and positive predictive values. Let's briefly forget about group labels -- we'll reintroduce them in a moment. 

First, the prevalence of positive outcomes is 
...
Ok, now it's time to do some algebra. Let's start with the $\mathrm{TPR}$ and see if we can find an equation that relates it to the $\mathrm{TPR}$. First, we'll multiply by $\frac{1 - \mathrm{PPV}}{\mathrm{PPV}}$. If we do this and insert the definitions of these quantities, we'll get 
